...
emitter.on('LangTaskRequired', async (fileID) => {
    ...
    const [[{ file: fileString }]] = await pool.execute("SELECT file FROM mdx_files WHERE id = ?;", [fileID]);
    fetch(process.env.AZURE_LANGUAGE_ENDPOINT + "/language/analyze-text/jobs?api-version=2023-04-01",
        ...
        .then(res => {
            if (!res.ok) {
                console.log(`Retrying upload for file with ID: ${fileID} due to non-ok response.`);
                setTimeout(() => {
                    emitter.emit("LangTaskRequired", fileBuffer, fileID);
                }, 10000)
                return;
            };
