...
export default async function Page({ params: { lang } }) {
    ...
    const disabledStyle = `${!session ? 'opacity-50 cursor-not-allowed pointer-events-none' : 'hover:cursor-pointer'}`

    return (
        <div className=" pt-6 grid gap-2">
            <div className="md:flex items-center">
                <section className="md:w-[200px] px-2">
                    <span className=" font-bold">Checkdown</span>{dict.HomePage.description}
                </section>
                <Demo />
            </div>
            <section>
                {dict.HomePage.uses} <Link href="https://nextjs.org/">Next.js</Link>, <Link href="https://socket.io/">Socket.IO</Link> {dict.HomePage.and} <Link href="https://mdxjs.com/">MDX</Link> {dict.HomePage.built}. {dict.HomePage.visit_src} <Link href="https://github.com/Yiipu/checkdown">Github</Link>.
            </section>
            <div className="md:flex">
                <section className="grid grid-cols-2 gap-2 pr-4 md:w-[600px]">
                    <label htmlFor="header-upload" className={`px-2 flex items-center justify-center bg-default rounded h-[50px] hover:cursor-pointer ${disabledStyle}`}>
                    ...
                    <label htmlFor="header-join" className={`px-2 flex items-center justify-center bg-default rounded h-[50px] hover:cursor-pointer ${disabledStyle}`}>
